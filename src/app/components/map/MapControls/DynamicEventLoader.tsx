...
// import { eventsLayerRef, GraphicRef, type CampusEvent } from "../arcgisRefs";
// import { toGraphic as toEventGraphic } from "./eventsLayer";
// import type { Mode, Unit } from "./EventCalendarOverlay";

// /**
//  * Fetches events from external endpoints (eventSources) for the active time window
//  * derived from the calendar/filter state, then injects them into the events layer.
//  * Removes previously-added dynamic graphics before each refetch to avoid duplicates.
//  */
// export default function DynamicEventLoader(props: {
//   eventSources: string[];
//   mode: Mode;
...
//   // Track previously-added dynamic graphics so we can remove them on next fetch
//   const dynamicGraphicsRef = useRef<any[]>([]);
//   const activeFetchAbortRef = useRef<AbortController | null>(null);
//   const debounceRef = useRef<number | null>(null);
...
//     if (!m) return null;
//     const [_, yy, mm, dd] = m;
//     return new Date(Number(yy), Number(mm) - 1, Number(dd), 0, 0, 0, 0);
//   }
...
//   function computeActiveWindow(): { from: Date; to: Date } {
//     const now = new Date();
//     let from = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 7);
//     let to = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 7);

//     if (mode === "single" && singleDate) {
//       const s = parseLocalDate(singleDate)!;
//       from = new Date(s.getFullYear(), s.getMonth(), s.getDate());
//       to = addInterval(from, 1, "day");
//     } else if (mode === "range" && (rangeStart || rangeEnd)) {
//       const s = parseLocalDate(rangeStart || rangeEnd)!;
//       const e = parseLocalDate(rangeEnd || rangeStart)!;
//       const lo = Math.min(s.getTime(), e.getTime());
//       const hi = Math.max(s.getTime(), e.getTime());
//       from = new Date(lo);
//       to = addInterval(new Date(hi), 1, "day");
//     } else if (mode === "timeline" && steps.length > 0) {
...
//     const { from, to } = computeActiveWindow();
//     const qs = (d: Date) => encodeURIComponent(d.toISOString());
...
//       // Expect { events: [{ id, title, start, end, lat, lon, description? }]} or an array
//       const list: any[] = Array.isArray(payload?.events)
//         ? payload.events
//         : Array.isArray(payload)
//         ? payload
...
//             names: undefined,
//             original: ev,
//             // Keep geometry in EPSG:4326; toEventGraphic will handle projection for the view
//             geometry: { x: lon, y: lat, wkid: 4326 },
//           };
...
//         activeFetchAbortRef.current = null;
//       }
//       onEventsUpdated?.(); // let parent recompute domain & re-apply filter
//     }
//   };

//   // Debounce refetch whenever the window-driving props change
//   useEffect(() => {
//     if (debounceRef.current) window.clearTimeout(debounceRef.current);
//     debounceRef.current = window.setTimeout(() => {
//       fetchDynamic();
