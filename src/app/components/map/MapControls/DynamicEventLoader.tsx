...
export default function DynamicEventLoader(props: {
  ...
  // --- NEW: .NET Date parser for "/Date(ms-tz)/" ---
  const parseDotNetDate = (dateStr: string) => {
    if (!dateStr) return null;
    // Extracts the milliseconds from the /Date(1234567890)/ format
    ...
    const requests = eventSources.map(async (baseUrl) => {
      ...
      const eventPromises = list.map(async (ev) => {
        try {
          ...
          // 1) Title - Added ev.Name for bus tracking data
          const title =
            ev.title ?? ev.name ?? ev.Name ?? ev.event_name ?? "Event";

          // 2) Start datetime
          let start: Date | null = null;

          // a) Try new .NET TimeStamp format first
          if (
            !start &&
            typeof ev.TimeStamp === "string" &&
            ev.TimeStamp.includes("/Date(")
          ) {
            const parsedDate = parseDotNetDate(ev.TimeStamp);
            if (parsedDate && !isNaN(parsedDate.getTime())) {
              start = parsedDate;
            }
          }

          // b) Try start_dt from backend (e.g. "2025-11-30T18:00:00")
          if (!start && typeof ev.start_dt === "string" && ev.start_dt) {
          ...
          // c) Fallback: combine date + start_at -> "YYYY-MM-DDTHH:MM:00"
          if (
            !start &&
          ...
          // d) LAST resort: generic `start` / `starts_at` fields
          ...
          // 3) Coordinates - Your existing code already handles ev.Latitude / ev.Longitude perfectly!
          let lat: number | null =
            ev.lat ??
            ev.latitude ??
            ev.Latitude ??
            (typeof ev.y === "number" ? ev.y : null);
          ...
          // Optional: Create a quick description for bus data if standard description is missing
          const fallbackDescription = ev.RouteID
            ? `Route: ${ev.RouteID} | Speed: ${Math.round(ev.GroundSpeed || 0)} mph | Delayed: ${ev.IsDelayed ? "Yes" : "No"}`
            : undefined;
