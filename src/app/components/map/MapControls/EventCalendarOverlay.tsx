...
export default function EventCalendarOverlay({
  ...
  // 🔧 more tolerant extraction of date/time from graphics
  function eventDateFromGraphic(g: any): Date | null {
    if (!g) return null;

    // Handle nested attribute patterns (g.attributes or g.attributes.attributes, etc.)
    const raw = g.attributes || g?.attributes?.attributes || g;
    ...
    const dateStr: string | undefined =
      ...
      (typeof a.event_date === "string" && a.event_date) ||
      (typeof a.original?.date === "string" && a.original.date) ||
      (typeof a.original?.event_date === "string" && a.original.event_date);
    ...
    const timeCandidate =
      ...
      a.original?.start_at ??
      a.original?.start ??
      a.original?.starts_at ??
      a.original?.time ??
      a.original?.timeStart;

    const timeStr =
      typeof timeCandidate === "string" && timeCandidate.trim().length > 0
        ? timeCandidate
        : null;

    return parseLocalDate(dateStr, timeStr);
  }
  ...
  const recomputeDomain = React.useCallback(() => {
    const layer = getLayer();
    const graphics: any[] = layer?.graphics?.toArray?.() ?? [];
  ...
  const applyFilter = React.useCallback(() => {
    const layer = getLayer();
    const items: any[] = layer?.graphics?.toArray?.() ?? [];
  ...
  // mount-only: attach listeners once
  React.useEffect(() => {
    // Try once immediately (in case layer is already ready AND populated)
    recomputeRef.current();
    filterRef.current();
    ...
    //
    // 🎯 *** ADD THIS LISTENER *** 🎯
    // Listen for the manual "change" event from DynamicEventLoader
    //
    const onLayerChange = () => {
      recomputeRef.current();
      filterRef.current();
    };
    eventsLayerRef?.events?.addEventListener("change", onLayerChange);
    ...
    return () => {
      ...
      // 🎯 *** ...AND CLEAN IT UP ***
      eventsLayerRef?.events?.removeEventListener("change", onLayerChange);
    };
...
//       const lyr = getLayer();
//       if (lyr?.graphics?.on) {
//         // Listen for future changes (DB events, external events, user events)
//         graphicsHandle = lyr.graphics.on("change", () => {
//           recomputeRef.current();
//           filterRef.current();
//         });

//         // ✅ IMPORTANT: recompute once *now* using whatever is already in the layer
//         recomputeRef.current();
//         filterRef.current();
//         return;
//       }

//       // Layer not ready yet; try again on next animation frame
//       raf = requestAnimationFrame(attachWhenReady);
//     }
