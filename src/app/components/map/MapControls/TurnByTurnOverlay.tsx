...
// type GeoJSONLine = {
//   type: "LineString";
//   coordinates: [number, number][]; // first and last define endpoints
// };
// type VertexFeature = {
//   type: "Feature";
//   id?: string | number;
...
//   hidden: boolean; // excluded from pickers & step text
//   lon: number;
//   lat: number;
...
//         // If the id is present in meta: default visible unless explicitly hidden:true
//         // If the id is NOT present in meta: hidden by default
...
//         // Prefer property Shape_Length if it looks like meters; else compute
...
//   /** Build human-friendly steps with turn classification */
//   const buildSteps = React.useCallback((path: Node[]): Step[] => {
//     if (path.length < 2) return [];

//     const out: Step[] = [];
//     let acc = 0;

//     // Always lead with a "Start" segment (no meters yet)
...
//       // At intermediate nodes, decide if there's a maneuver
//       if (i < path.length - 1) {
...
//         if (kind !== "straight") {
//           const at =
//             !path[i].hidden && path[i].name ? ` at ${path[i].name}` : "";
//           const text =
...
//           acc >= 1000 ? (acc / 1000).toFixed(2) + " km" : Math.round(acc) + " m"
//         }`,
//         meters: acc,
//       });
//     }
